...
let rec find_match : pattern -> value -> env option = fun p v ->
  ...
  | PTCons (a,arest),VTCons (v,vrest) -> moveopt ((find_match a v)::[find_match arest vrest])
  | PLCons (a,arest),VLCons (v,vrest) -> moveopt ((find_match a v)::[find_match arest vrest])
  | PNil,VNil -> Some []
  | PNil,_ -> None
  | PVar px,v -> Some [(px,v)]
  | PUnder,v -> Some [("-",v)]
...
and find_t_elm pconf vconf = 
  match pconf,vconf with
    PNil,VNil -> []
  | PTCons(p,prest),VTCons(v,vrest) -> 
    (find_match p v):: (find_t_elm prest vrest)
  | _ -> raise (Eval_error (__LOC__))

and find_l_elm pconf vconf = 
  match pconf,vconf with
    PNil,VNil -> []
  | PLCons(p,prest),VLCons(v,vrest) -> 
    (find_match p v):: (find_l_elm prest vrest)
  | _ -> raise (Eval_error (__LOC__))
...
and eval : env -> expr -> value = fun ev ex ->
  ...
  | EApp (e1,e2) ->
    ...
    begin
      match v1 with 
        VFun (p,e,oenv) -> eval (get(find_match p v2)@oenv) e
      | VRFun (n,lst,oenv) -> let (_,p',e') = nth_name lst n in
        let rec make_value l=
          begin
            match l with
              [] -> []
        ...
        in let env'= get(find_match p' v2)@make_env lst (make_value lst) @oenv in eval env' e'
