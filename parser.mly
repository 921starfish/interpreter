...
command:   
  ...
  | LET pattern patterns DSEMICOLON {CLet($2,$3)}
  | LET REC var pattern EQ expr ands DSEMICOLON { CRLet (($3,$4,$6)::$7) }
  | LET REC var pattern patterns ands DSEMICOLON { CRLet(($3,$4,$5)::$6) }
  | EOF { EoF }
;

ands:
  |       {[]}
  | AND var pattern EQ expr ands { ($2,$3,$5)::$6 } 
  | AND var pattern patterns ands { ($2,$3,$4)::$5 } 
;

patterns:
  | pattern EQ expr { EFun ($1,$3) }
  | pattern patterns { EFun ($1,$2) }
;

patternf:
  | pattern RARROW expr                 { EFun ($1,$3) }
  | pattern patternf                        { EFun ($1,$2) }
;
    
expr:
  | FUN pattern RARROW expr       { EFun ($2,$4) }
  | FUN pattern patternf              { EFun ($2,$3) }
  | LET REC var pattern EQ expr ands IN expr { ERLet(($3,$4,$6)::$7,$9) }
  | LET REC var pattern patterns ands IN expr { ERLet(($3,$4,$5)::$6,$8) }
  | LET pattern EQ expr IN expr   { ELet($2,$4,$6) }
  | LET pattern patterns IN expr {ELet($2,$3,$5)}
